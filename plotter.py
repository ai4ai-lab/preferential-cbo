...
class Plotter():
    ...
    def plot_moon(self, target, prefflow, data, cfg):
        if self.d > 2:
            print(f"[plot_moon] Skipping 2D contour plot because D = {self.d} > 2.")
            
            if prefflow is not None:
                prefflow.eval()
                with torch.no_grad():
                    samples = prefflow.sample(1000)[0].cpu().numpy()
                from sklearn.decomposition import PCA
                projected = PCA(n_components=2).fit_transform(samples)
                plt.figure(figsize=(8, 8))
                plt.scatter(projected[:, 0], projected[:, 1], alpha=0.5, label="PNF samples (PCA)")
                plt.title(f"PCA projection of samples in {self.d}D")
                plt.legend()
                plt.show()
            return None
