...
class PrefFlow(NormalizingFlow):
    ...
    def extract_preferred(self,batch,X,logf):
    ...
    def logposterior(self, batch, weightprior=1.0):
        ...
        #LIKELIHOOD PAIRWISE
        if not self.ranking:
            loglik += torch.sum(self.loglik_pairwise(logf, Y), dim=0)
        #LIKELIHOOD RANKING
        if self.ranking:
            logf = logf.view(k, n)
        ...
        #Prior
        if not self.ranking:
            winners_logf, _ = self.extract_preferred((X_raw, Y), X, logf) #preferred points of pairwise comparisons
        logpx = winners_logf
        logprior = logpx.sum()
